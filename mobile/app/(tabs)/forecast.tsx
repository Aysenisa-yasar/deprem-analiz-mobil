...
function quakeSourceLabel(event: QuakeEvent | null) {
...
export default function ForecastScreen() {
  ...
  const filteredPoints = useMemo(
    () => filterPoints(sortedPoints, deferredQuery),
    [deferredQuery, sortedPoints]
  );
  const topPoints = useMemo(() => filteredPoints.slice(0, 3), [filteredPoints]);
  const focusPoint = useMemo(() => {
    const selected = selectedCity
      ? filteredPoints.find((point) => point.city === selectedCity)
      : null;
  ...
  const trendBars = useMemo(() => {
    ...
    const avg =
      sample.reduce((sum, point) => sum + riskValue(point.risk_score), 0) / sample.length;
    return sample.map(
      (point, index) => riskValue(point.risk_score) - avg + (index % 2 === 0 ? 0.08 : -0.04)
    );
  }, [filteredPoints]);

  useEffect(() => {
    if (!selectedCity) return;
    if (!filteredPoints.some((point) => point.city === selectedCity)) {
  ...
  const loadBase = useCallback(async () => {
    ...
    const hasStatusData =
      ...
      Boolean(statusRes.value.modelHealth || statusRes.value.warningCapability);
    ...
    if (!hasMapData && !hasStatusData) {
      ...
      setError('Canli risk katmani gecikiyor. Son deprem ve model ozeti gosteriliyor.');
  ...
  const loadLocation = useCallback(
    async (requestPermission: boolean) => {
      try {
        setLocating(true);
        const location = await getSafeDeviceLocation({
          requestPermission,
          allowLastKnown: true,
        });
        if (!location.ok) {
            ...
            <View style={styles.headerBlock}>
              <View style={styles.headerTop}>
                <View style={{ flex: 1 }}>
                  <Text style={[styles.headerTitle, { color: t.text }]}>
                    Turkiye Deprem Risk Haritasi
                  </Text>
                  <Text style={[styles.headerSub, { color: t.textSecondary }]}>
                    Kandilli destekli son deprem akisi, il bazli risk puani ve isi katmani tek
                    ekranda.
                  </Text>
                </View>
              ...
              <GlassCard t={t} style={styles.searchCard}>
                ...
                <View style={styles.legendRow}>
                  {LEGEND.map((item) => {
                    const accent =
                      item.key === 'high' ? t.high : item.key === 'mid' ? t.mid : t.low;
                    return (
                      <View
                        key={item.key}
                        style={[
                        ...
                        <Text style={[styles.legendText, { color: t.text }]}>{item.label}</Text>
                      </View>
                    );
                  })}
                </View>
              ...
              <GlassCard t={t} tone="warm" style={styles.card}>
                <View style={styles.rowBetween}>
                  <CosmicLabel t={t} accent={t.high}>
                    risk haritasi
                  </CosmicLabel>
                  <Text style={[styles.meta, { color: t.textSecondary }]}>
                    Isi katmani + il etiketleri
                  </Text>
                </View>
                  ...
                  {topPoints.map((item) => {
                    ...
                    return (
                      <Pressable
                        ...
                        <Text style={[styles.pillTitle, { color: accent }]}>{item.city}</Text>
                        <Text style={[styles.pillMeta, { color: t.textSecondary }]}>
                          %{Math.round(riskValue(item.risk_score) * 100)}
                        </Text>
                      </Pressable>
                    );
                  })}
                ...
                <View style={styles.mapWrap}>
                  <RiskMap
                    points={filteredPoints.length ? filteredPoints : sortedPoints}
                    heatPoints={heatPoints}
                    scheme={scheme}
                    t={t}
                    userLocation={userLocation}
                    focusPoint={
                      focusPoint ? { lat: focusPoint.lat, lon: focusPoint.lon } : userLocation
                    }
                  />
                  <View
                    style={[
                      styles.overlay,
                      { backgroundColor: t.glassOverlay, borderColor: t.border },
                    ]}>
                    <Text style={[styles.overlayTitle, { color: t.text }]}>
                      {focusPoint?.city || 'Harita odagi'}
                    </Text>
                    <Text style={[styles.overlayBody, { color: t.textSecondary }]}>
                      {focusPoint
                        ? `${focusPoint.risk_level} risk - M4+ ${percent(focusPoint.probability)}`
                        : 'Bir il secerek detay kartini goster.'}
                    </Text>
                  </View>
                </View>
              </GlassCard>
              ...
              <GlassCard t={t} tone="cool" style={styles.card}>
                <View style={styles.rowBetween}>
                  ...
                  <CosmicLabel t={t} accent={t.warn}>
                    {recentQuakes.length} olay
                  </CosmicLabel>
                </View>
                <View style={styles.quakeWrap}>
                  <QuakeMap events={recentQuakes} scheme={scheme} t={t} />
                </View>
                <Text style={[styles.meta, { color: t.textSecondary }]}>
                  {recentQuakes[0]
                    ? `${quakeSourceLabel(recentQuakes[0])} - M${recentQuakes[0].mag.toFixed(1)} - ${recentQuakes[0].depth.toFixed(0)} km - ${relativeTimeTr(recentQuakes[0].timestamp)}`
                    : 'Son deprem verisi bekleniyor.'}
                </Text>
              </GlassCard>
                ...
                <GlassCard t={t} tone="warm" style={styles.gridCard}>
                  ...
                  <Text
                    style={[
                      ...
                      {
                        color: focusPoint
                          ? riskAccent(scheme, focusPoint.risk_level, focusPoint.risk_score)
                          : t.warn,
                      },
                    ]}>
                    %{Math.round(riskValue(focusPoint?.risk_score) * 100)}
                  </Text>
                  <MiniBars
                    t={t}
                    values={trendBars}
                    positiveColor={alpha(t.success, 0.9)}
                    negativeColor={alpha(t.high, 0.82)}
                  />
                </GlassCard>
                <GlassCard t={t} tone="cool" style={styles.gridCard}>
                  <Text style={[styles.cardTitle, { color: t.text }]}>Model Hazirligi</Text>
                  <Text style={[styles.bigValue, { color: t.glowBlue }]}>
                    {modelLabel(modelHealth)}
                  </Text>
                  <Text style={[styles.meta, { color: t.textSecondary }]}>
                    ROC-AUC {modelHealth?.metrics?.roc_auc_mean?.toFixed(2) ?? '--'} - Precision{' '}
                    {percent(modelHealth?.backtest?.precision)}
                  </Text>
                  <Text style={[styles.meta, { color: t.textSecondary }]}>
                    Son egitim: {trainedAt(modelHealth?.trained_at)}
                  </Text>
                </GlassCard>
              </View>
              ...
              <GlassCard t={t} style={styles.card}>
                ...
                {locating ? (
                  ...
                  <Text style={[styles.meta, { color: t.textSecondary }]}>
                    {locationPoint
                      ? `Risk ${riskValue(locationPoint.risk_score).toFixed(2)} - M5+ ${percent(locationPoint.m5_72h_probability)} - Fay ${locationPoint.fault_distance != null ? `${locationPoint.fault_distance.toFixed(0)} km` : '--'}`
                      : 'Konum izni ile kisisel risk karti gosterilir.'}
                  </Text>
                )}
                {locationMessage ? (
                  <Text style={[styles.meta, { color: t.textSecondary }]}>
                ...
                <View style={styles.actions}>
                  <GlowButton
                    t={t}
                    label={locating ? 'Konum aliniyor...' : 'Konum riskimi hesapla'}
              ...
              <GlassCard t={t} tone="cool" style={styles.card}>
                ...
                <Text style={[styles.meta, { color: t.textSecondary }]}>
                  {warningCapability?.summary ||
                    'ML tabanli risk uyarilari hazir. Resmi sensor tabanli saniyeler-once erken uyari bu surumde yok.'}
                </Text>
              </GlassCard>
                ...
                <GlassCard
                  ...
                  <View style={styles.rowBetween}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={[styles.meta, { color: t.textSecondary }]}>
                        Risk {riskValue(item.risk_score).toFixed(2)} - M4+ {percent(item.probability)} -
                        Fay {item.fault_distance != null ? ` ${item.fault_distance.toFixed(0)} km` : ' --'}
                      </Text>
                    </View>
                    <CosmicLabel t={t} accent={accent}>
                      {item.risk_level}
                    </CosmicLabel>
                  </View>
                </GlassCard>
